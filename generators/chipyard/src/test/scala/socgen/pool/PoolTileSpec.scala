...
class PoolTileSpec extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "bind first, interior and tail regions without changing the template" in {
    test(new PoolTileBinding(PoolParams(elementBits = 8), auto)) { dut =>
      ...
      dut.io.request.sourceTile.column.poke(8.U)
      dut.io.valid.expect(false.B)
      dut.io.request.sourceTile.column.poke(9.U)
      dut.io.request.bytes.poke(11.U)
      dut.io.valid.expect(false.B)
      dut.io.request.bytes.poke(12.U)
      dut.io.request.sourceTile.id.poke(4.U)
      dut.io.valid.expect(false.B)
      dut.io.request.sourceTile.id.poke(3.U)
      dut.io.configured.destination.poke(((BigInt(1) << 64) - 16).U)
      dut.io.valid.expect(false.B)
    }
  }

  it should "report an invalid view without issuing a streaming job" in {
    test(new PoolLinkAdapter(PoolParams(elementBits = 8), Some(PoolLinkParams(auto)))) { dut =>
      ...
      tile(port.requestCopy.bits.sourceTile, 1, 0, 4, 4)
      ...
      dut.io.job.valid.expect(false.B)
      dut.clock.step()
      port.requestCopy.valid.poke(false.B)
      port.reportCopy.valid.expect(true.B)
      port.reportCopy.bits.detail.expect(PoolStatus.BadLength)
