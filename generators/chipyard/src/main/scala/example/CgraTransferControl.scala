...
/** Locks validation and production control to one source per reset. Descriptor
  * and launch-header selection share the lock, so a blocked completion cannot
  * be reattributed by a later descriptor from the other source.
  */
class CgraTransferControlSourceSelector(
  params: CgraComputeLaunchGateParams) extends Module {
  private val consumerParams = CgraConsumerPullAdapterParams.production

  val io = IO(new Bundle {
    val productionDescriptorIn = Flipped(Decoupled(
      new CgraConsumerPullDescriptor(consumerParams)))
    val validationDescriptorIn = Flipped(Decoupled(
      new CgraConsumerPullDescriptor(consumerParams)))
    val descriptorOut = Decoupled(
      new CgraConsumerPullDescriptor(consumerParams))
    val productionLaunchHeaderIn = Flipped(Decoupled(
      new CgraLaunchSequenceHeader(params)))
    val validationLaunchHeaderIn = Flipped(Decoupled(
      new CgraLaunchSequenceHeader(params)))
    val launchHeaderOut = Decoupled(new CgraLaunchSequenceHeader(params))
    val completionIn = Flipped(Decoupled(
      new CgraConsumerCompletion(consumerParams)))
    val productionCompletionOut = Decoupled(
      new CgraConsumerCompletion(consumerParams))
    val validationCompletionOut = Decoupled(
      new CgraConsumerCompletion(consumerParams))
    val selected = Output(Bool())
    val productionSelected = Output(Bool())
  })

  val selected = RegInit(false.B)
  val productionSelected = RegInit(false.B)
  val productionHasWork = io.productionDescriptorIn.valid ||
    io.productionLaunchHeaderIn.valid
  val validationHasWork = io.validationDescriptorIn.valid ||
    io.validationLaunchHeaderIn.valid
  val chooseProduction = Mux(
    selected,
    productionSelected,
    productionHasWork || !validationHasWork)

  io.descriptorOut.valid := Mux(
    chooseProduction,
    io.productionDescriptorIn.valid,
    io.validationDescriptorIn.valid)
  io.descriptorOut.bits := Mux(
    chooseProduction,
    io.productionDescriptorIn.bits,
    io.validationDescriptorIn.bits)
  io.productionDescriptorIn.ready :=
    io.descriptorOut.ready && chooseProduction
  io.validationDescriptorIn.ready :=
    io.descriptorOut.ready && !chooseProduction

  io.launchHeaderOut.valid := Mux(
    chooseProduction,
    io.productionLaunchHeaderIn.valid,
    io.validationLaunchHeaderIn.valid)
  io.launchHeaderOut.bits := Mux(
    chooseProduction,
    io.productionLaunchHeaderIn.bits,
    io.validationLaunchHeaderIn.bits)
  io.productionLaunchHeaderIn.ready :=
    io.launchHeaderOut.ready && chooseProduction
  io.validationLaunchHeaderIn.ready :=
    io.launchHeaderOut.ready && !chooseProduction

  when((io.descriptorOut.fire || io.launchHeaderOut.fire) && !selected) {
    selected := true.B
    productionSelected := chooseProduction
  }

  io.productionCompletionOut.valid :=
    io.completionIn.valid && selected && productionSelected
  io.productionCompletionOut.bits := io.completionIn.bits
  io.validationCompletionOut.valid :=
    io.completionIn.valid && selected && !productionSelected
  io.validationCompletionOut.bits := io.completionIn.bits
  io.completionIn.ready := selected && Mux(
    productionSelected,
    io.productionCompletionOut.ready,
    io.validationCompletionOut.ready)

  io.selected := selected
  io.productionSelected := productionSelected

  when(io.completionIn.valid) {
    assert(selected,
      "consumer completion observed before control-source selection")
  }
}
